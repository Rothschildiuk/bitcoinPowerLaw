...
st.markdown(f"""
    ...
    /* Приховування кнопок Deploy та Menu, але збереження хедера для стрілки сайдбару */
    ...
    [data-testid="stSidebar"] {{ width: 320px !important; background-color: {c_sidebar_bg} !important; border-right: 1px solid {c_border}; }}
    ...
    /* Sidebar Small Settings */
    .small-settings {{ font-size: 11px !important; opacity: 0.7; margin-top: 20px; }}
    div[data-testid="stSidebar"] .stRadio div[role="radiogroup"] {{ gap: 5px; }}
    div[data-testid="stSidebar"] .stRadio label p {{ font-size: 12px !important; }}
    ...
    .stButton > button:hover {{ background-color: {c_btn_hover} !important; border-color: {c_text_main} !important; color: {c_text_val} !important; }}
...
# Initialize defaults
for k, v in {"genesis_offset": int(opt_offset), "A": float(round(opt_A, 3)), "B": float(round(opt_B, 3)),
...
# --- SIDEBAR UI ---
with st.sidebar:
    st.markdown(f"<h2 style='text-align: center; color: #f0b90b; margin-bottom: 20px;'>{T['title']}</h2>",
                unsafe_allow_html=True)

    c_v1, c_v2 = st.columns(2)
    price_scale = c_v1.radio(T['price_scale'], ["Log", "Lin"], index=0)
    time_scale = c_v2.radio(T['time_scale'], ["Log", "Lin"], index=0)

    st.markdown(
        f"<p style='color:{c_text_main}; text-align:center; font-size: 0.85rem;'>{T['max_r2']}: {opt_R2 * 100:.3f}%<br>({T['offset_txt']} {opt_offset}d)</p>",
        unsafe_allow_html=True)
...
m_fair_usd = 10 ** (A + B * m_log_d)

fig = make_subplots(rows=2, cols=1, shared_xaxes=False, vertical_spacing=0.10, row_heights=[0.76, 0.24])
fig.add_trace(go.Scatter(x=m_x, y=10 ** (A + B * m_log_d + p97_5), mode='lines', line=dict(width=0), showlegend=False,
                         hoverinfo='skip'), 1, 1)
fig.add_trace(go.Scatter(x=m_x, y=10 ** (A + B * m_log_d + p83_5), mode='lines', line=dict(width=0), fill='tonexty',
                         fillcolor='rgba(234, 61, 47, 0.15)', name=T['leg_bubble'], customdata=m_dates_str,
                         hovertemplate=f"{T['hover_date']}: %{{customdata}}<br>{T['hover_price']}: $%{{y:,.0f}}<extra></extra>"),
              1, 1)
fig.add_trace(go.Scatter(x=m_x, y=10 ** (A + B * m_log_d + p16_5), mode='lines', line=dict(width=0), showlegend=False,
                         hoverinfo='skip'), 1, 1)
fig.add_trace(go.Scatter(x=m_x, y=10 ** (A + B * m_log_d + p2_5), mode='lines', line=dict(width=0), fill='tonexty',
                         fillcolor='rgba(14, 203, 129, 0.15)', name=T['leg_accum'], customdata=m_dates_str,
                         hovertemplate=f"{T['hover_date']}: %{{customdata}}<br>{T['hover_price']}: $%{{y:,.0f}}<extra></extra>"),
              1, 1)
fig.add_trace(go.Scatter(x=m_x, y=m_fair_usd, mode='lines', line=dict(color='#f0b90b', width=1.5, dash='dash'),
                         ...
                         hovertemplate=f"{T['hover_date']}: %{{customdata}}<br>{T['hover_fair']}: $%{{y:,.0f}}<extra></extra>"),
              1, 1)
fig.add_trace(
    go.Scatter(x=df['Days'], y=df['Close'], mode='lines', name=T['leg_price'], line=dict(color=pl_btc_color, width=1.3),
...
for r in [1, 2]:
    fig.update_xaxes(type="log" if time_scale == "Log" else "linear", tickvals=t_vals, ticktext=t_text,
                     range=[np.log10(t_vals[0]), np.log10(view_max)] if time_scale == "Log" else [0, view_max],
                     gridcolor=pl_grid_color, tickfont=dict(color=pl_text_color, size=13), row=r, col=1)
fig.update_yaxes(type="log" if price_scale == "Log" else "linear",
                 ...
                 gridcolor=pl_grid_color, tickfont=dict(color=pl_text_color, size=13), row=1, col=1)

fig.add_trace(
    go.Scatter(x=df['Days'], y=df['Res'], mode='lines', name=T['leg_osc'], line=dict(color='#0ecb81', width=1.2),
               customdata=df.index.strftime('%d.%m.%Y'),
               hovertemplate=f"{T['hover_date']}: %{{customdata}}<br>{T['hover_osc']}: %{{y:.3f}}<extra></extra>"), 2,
    1)
fig.add_hline(y=0, line_width=1, line_color=pl_legend_color, row=2, col=1)
for i in range(6):
    fig.add_vline(x=t1_age * (lambda_val ** i) * 365.25, line_width=1.5, line_dash="dash", line_color="#ea3d2f",
                  opacity=0.8, row=2, col=1)
    fig.add_vline(x=t1_age * (lambda_val ** (i + 0.5)) * 365.25, line_width=1, line_dash="dot", line_color="#2b6aff",
                  opacity=0.5, row=2, col=1)

fig.update_layout(height=720, margin=dict(t=30, b=10, l=50, r=20), template=pl_template, font=dict(color=pl_text_color),
                  legend=dict(orientation="h", y=0.27, x=0.5, xanchor="center",
                              font=dict(size=15, color=pl_legend_color)), paper_bgcolor=pl_bg_color,
...
diff, pot = ((l_p - l_f) / l_f) * 100, ((10 ** (A + B * np.log10(df['Days'].max()) + p97_5) - l_p) / l_p) * 100
