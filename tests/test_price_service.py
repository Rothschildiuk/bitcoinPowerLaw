...
class TestPriceService(unittest.TestCase):
    ...
    def test_build_currency_close_series_for_uah(self, mock_load_reference_series):
        ...
        self.assertListEqual(result.round(6).tolist(), [4000.0, 4920.0])

    @patch("services.price_service.load_reference_series")
    def test_build_currency_close_series_for_chf(self, mock_load_reference_series):
        idx = pd.to_datetime(["2024-01-01", "2024-01-02"])
        raw_df = pd.DataFrame({"Close": [100.0, 120.0]}, index=idx)
        eur_usd = pd.Series(dtype=float)
        ...
        result = price_service.build_currency_close_series(raw_df, "CHF")
    ...
    def test_build_currency_close_series_for_silver(self, mock_load_reference_series):
        ...
        self.assertListEqual(result.round(6).tolist(), [4.0, 4.0])

    @patch("services.price_service.load_reference_series")
    def test_build_currency_close_series_for_copper(self, mock_load_reference_series):
        idx = pd.to_datetime(["2024-01-01", "2024-01-02"])
        raw_df = pd.DataFrame({"Close": [100.0, 120.0]}, index=idx)
        eur_usd = pd.Series(dtype=float)
        usd_uah = pd.Series(dtype=float)
        ...
        self.assertListEqual(result.round(6).tolist(), [25.0, 24.0])

    @patch("services.price_service.load_reference_series")
    def test_build_currency_close_series_for_iron(self, mock_load_reference_series):
        idx = pd.to_datetime(["2024-01-01", "2024-01-02"])
        raw_df = pd.DataFrame({"Close": [100.0, 120.0]}, index=idx)
        eur_usd = pd.Series(dtype=float)
        usd_uah = pd.Series(dtype=float)
        ...
        self.assertListEqual(result.round(6).tolist(), [1.0, 1.5])

    @patch("services.price_service.load_reference_series")
    def test_build_currency_close_series_for_aluminum(self, mock_load_reference_series):
        idx = pd.to_datetime(["2024-01-01", "2024-01-02"])
        raw_df = pd.DataFrame({"Close": [100.0, 120.0]}, index=idx)
        eur_usd = pd.Series(dtype=float)
        usd_uah = pd.Series(dtype=float)
    ...
    @patch("services.price_service.load_reference_series")
    def test_build_currency_close_series_for_sp500(self, mock_load_reference_series):
        idx = pd.to_datetime(["2024-01-01", "2024-01-02"])
        ...
        oil_usd = pd.Series(dtype=float)
        us_housing = pd.Series(dtype=float)
        sp500 = pd.Series([5000.0, 6000.0], index=idx)
        mock_load_reference_series.return_value = (
            eur_usd,
            usd_uah,
            usd_rub,
